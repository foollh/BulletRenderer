...
class Camera:
    def __init__(self, resolution=(320, 240), near=0.01, far=10, client_id=0):
        ...
        mat = transforms3d.euler.euler2mat(np.pi / 2, 0, 0, axes='rxyz')
        self.TCCGL = transforms3d.affines.compose(
            T=[0, 0, 0],
            R=mat,
            Z=np.ones(3),
        )
    ...
    def set_extrinsic_T(self, TWC):
        ...
        # wxyz = transforms3d.affines.decompose(TWCGL)
        # xyzw = TWCGL.quaternion.coeffs()
        # wxyz = [xyzw[-1], *xyzw[:-1]]
        pitch, roll, yaw = transforms3d.euler.mat2euler(TWCGL[:3, :3], axes='sxyz')
        yaw = yaw * 180 / np.pi
        pitch = pitch * 180 / np.pi
        roll = roll * 180 / np.pi
        yaw = (yaw % 360 + 360) % 360
        distance = 0.0001
        self.set_extrinsic_bullet(target=TWCGL[:3, -1], distance=distance,
    ...
    def get_state(self):
        ...
        mat = transforms3d.euler.euler2mat(*[self._view_params[k]*np.pi/180 for k in ('pitch', 'roll', 'yaw')], axes='sxyz')
        TWCGL = transforms3d.affines.compose(
            T=trans,
            R=mat,
            Z=np.ones(3),
        )
        # wxyz = transforms3d.euler.euler2quat([self._view_params[k]*np.pi/180 for k in ('pitch', 'roll', 'yaw')], axes='sxyz')
        # orn = np.array([*wxyz[1:], wxyz[0]])
        # orn = euler2quat([self._view_params[k]*np.pi/180 for k in ('pitch', 'roll', 'yaw')], axes='sxyz')
        # TWCGL = Transform(orn, trans)
        TWC = TWCGL * np.linalg.inv(self.TCCGL) ################################################################
        obs.update(TWC=TWC, K=K, resolution=(self._shape[1], self._shape[0]),
